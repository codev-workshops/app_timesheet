...
describe('Database Initialization', () => {
  ...
  describe('getDatabase', () => {
    ...
    test('should handle database connection error', () => {
      ...
      const {
        getDatabase: getDatabaseWithError,
      } = require('../../database/init');
  ...
  describe('initializeDatabase', () => {
    test('should create all required tables', async () => {
      ...
      expect(
        queries.some((q) => q.includes('CREATE TABLE IF NOT EXISTS users')),
      ).toBe(true);
      expect(
        queries.some((q) => q.includes('CREATE TABLE IF NOT EXISTS clients')),
      ).toBe(true);
      expect(
        queries.some((q) =>
          q.includes('CREATE TABLE IF NOT EXISTS work_entries'),
        ),
      ).toBe(true);
    });
    ...
    test('should create indexes for performance', async () => {
      ...
      expect(
        queries.some((q) =>
          q.includes('CREATE INDEX IF NOT EXISTS idx_clients_user_email'),
        ),
      ).toBe(true);
      expect(
        queries.some((q) =>
          q.includes('CREATE INDEX IF NOT EXISTS idx_work_entries_client_id'),
        ),
      ).toBe(true);
      expect(
        queries.some((q) =>
          q.includes('CREATE INDEX IF NOT EXISTS idx_work_entries_user_email'),
        ),
      ).toBe(true);
      expect(
        queries.some((q) =>
          q.includes('CREATE INDEX IF NOT EXISTS idx_work_entries_date'),
        ),
      ).toBe(true);
    });
  ...
  describe('Database Schema', () => {
    test('users table should have correct structure', async () => {
      ...
      expect(userTableQuery[0]).toContain(
    ...
    test('clients table should have foreign key to users', async () => {
      ...
      expect(clientTableQuery[0]).toContain(
    ...
    test('work_entries table should have foreign keys', async () => {
      ...
      expect(workEntriesQuery[0]).toContain(
