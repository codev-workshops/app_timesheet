...
export default function DashboardPage() {
  ...
  const totalHours = workEntries.reduce(
    (sum: number, entry: Record<string, unknown>) => sum + (entry.hours as number),
    0
  );
            ...
            {recentEntries.length > 0 ? (
              recentEntries.map(
                (entry: Record<string, unknown>) => (
                  ...
                  >
                    <Typography variant="subtitle1">{entry.client_name as string}</Typography>
                    <Typography variant="body2" color="text.secondary">
                      {entry.hours as number} hours -{' '}
